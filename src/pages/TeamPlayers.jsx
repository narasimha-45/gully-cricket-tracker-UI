...
/**
 * Squad selection screen, shared by both teams.
 * teamKey: "teamA" | "teamB" - which side of `match.teams` this screen edits.
 */
export default function TeamPlayers({ teamKey }) {
    ...
    const fetchPlayers = async () => {
      try {
        ...
        const res = await fetch(
          `${API_ENDPOINTS.SEARCH_PLAYER}?query=${encodeURIComponent(query)}`,
        );
        const json = await res.json();
        if (res.status == 200) setResults(json || []);
      } catch (err) {
  ...
  const players = team.players.map((p) => p.toLowerCase().trim()) || [];
  ...
  const addPlayer = async (name) => {
    ...
    const updated = {
      ...
      teams: {
        ...match.teams,
        [teamKey]: { ...team, players: [...players, name.trim()] },
      },
      updatedAt: Date.now(),
    };
  ...
  const removePlayer = async (index) => {
    const updated = {
      ...
      teams: {
        ...match.teams,
        [teamKey]: { ...team, players: players.filter((_, i) => i !== index) },
      },
      updatedAt: Date.now(),
    };
    await saveMatch(updated);
  ...
  return (
    <div style={container}>
      {/* HEADER */}
      <div style={header}>
        <button onClick={() => navigate(-1)} style={backCircle}>
          ←
        </button>
        <div style={{ flex: 1 }}>
            ...
            {results.length > 0 ? (
              results.map((p, idx) => (
                <div
                  key={idx}
                  style={dropItem}
                  onClick={() => addPlayer(p.playerName)}
                >
        ...
        {players.length === 0 ? (
          ...
          players.map((p, i) => (
            <div key={i} style={playerRow}>
              ...
              <button onClick={() => removePlayer(i)} style={removeBtn}>
                ✕
              </button>
            </div>
          ))
