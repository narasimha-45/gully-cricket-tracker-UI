...
export default function TeamAPlayers() {
    ...
    const loadMatch = async () => {
      try {
        ...
        if (m) {
          ...
          const hasIds = teamA.players.some(p => /^[0-9a-fA-F]{24}$/.test(p));
          const needsSync = hasIds || (teamA.isExisting && teamA.players.length === 0);
          
          if (needsSync) {
            try {
              const res = await fetch(`${API}/api/teams/${encodeURIComponent(teamA.name)}`);
  ...
  const addPlayer = async (name) => {
    ...
    const updated = {
      ...
      teams: {
        ...match.teams,
        teamA: { ...team, players: [...players, name.trim()] },
      },
      updatedAt: Date.now(),
    };
  ...
  const removePlayer = async (index) => {
    const updated = {
      ...
      teams: {
        ...match.teams,
        teamA: { ...team, players: players.filter((_, i) => i !== index) },
      },
      updatedAt: Date.now(),
    };
    await saveMatch(updated);
  ...
  return (
    <div style={container}>
      ...
      <div style={footer}>
        ...
        >
          Next: {formatName(match.teams.teamB.name)} →
        </button>
      </div>
    </div>
  );
