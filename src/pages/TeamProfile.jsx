...
export default function TeamProfile() {
  ...
  if (profileQuery.isLoading && !profile) return <LoadingState label="Loading team profile…" />;
      ...
      {profile && (
        <div className={styles.page}>
          <section className={styles.headerCard}>
            <div className={styles.headerTop}>
              <div className={styles.identity}>
                <div className={styles.avatar}>{(profile.teamName || "T").slice(0, 1).toUpperCase()}</div>
                <div className={styles.titleWrap}>
                  <p className={styles.eyebrow}>Team profile</p>
                  <h1 className={styles.title}>{formatName(profile.teamName)}</h1>
                  <p className={styles.subtitle}>{seasonId ? "Season view" : "All-time view"}</p>
                </div>
              </div>
              <select className={styles.select} value={seasonId} onChange={(event) => setSeasonId(event.target.value)} aria-label="Filter team profile by season">
          ...
          <section className={styles.section}>
            <SectionHeader title="Match approach" subtitle="Bat first vs chase" />
            <div className={styles.statGrid}>
              <StatTile label="Batted first" value={n(profile.timesBattedFirst)} />
              <StatTile label="Wins batting first" value={n(profile.timesWonBattingFirst)} />
              <StatTile label="Win % batting first" value={`${d(profile.winPercentageBattingFirst)}%`} />
              <StatTile label="Chases" value={n(profile.timesBattedSecond)} />
              <StatTile label="Chase wins" value={n(profile.timesWonChasing)} />
              <StatTile label="Chase win %" value={`${d(profile.winPercentageChasing)}%`} />
            </div>
          </section>
          ...
          <section className={styles.section}>
            ...
            <div className={styles.statGrid}>
              <StatTile label="Runs scored" value={n(profile.totalRunsScored)} />
              <StatTile label="Runs conceded" value={n(profile.totalRunsConceded)} />
          ...
          <section className={styles.section}>
            <SectionHeader title="Records" subtitle="Notable completed matches" />
            <div className={styles.list}>
              <Notable label="Highest score" match={profile.highestTeamScore} />
              <Notable label="Lowest score" match={profile.lowestTeamScore} />
              <Notable label="Lowest defended" match={profile.lowestTotalDefended} />
              <Notable label="Highest chased" match={profile.highestTotalChased} />
            </div>
          </section>

          <section className={styles.section}>
            <SectionHeader title="Recent matches" subtitle="Latest results" />
            {(profile.recentMatches || []).length === 0 ? <div className={styles.empty}>No recent matches available.</div> : (
              <div className={styles.list}>
                {profile.recentMatches.map((match) => (
                  <div key={match.matchId} className={styles.listRow}>
                    <strong>vs {formatName(match.opponentTeamName)}</strong>
                    <span>{n(match.teamScore)}/{n(match.teamWickets)}</span>
                    <span>{n(match.opponentScore)}/{n(match.opponentWickets)}</span>
          ...
          {!seasonId && (profile.bySeason || []).length > 0 && (
            <section className={styles.section}>
              <SectionHeader title="Season history" subtitle="Team progression" />
              <div className={styles.list}>
...
function Notable({ label, match }) {
  if (!match) return <div className={styles.notable}><strong>{label}</strong><span>—</span><span>—</span><span>—</span></div>;
  return <div className={styles.notable}><strong>{label}</strong><span>{n(match.teamScore)}/{n(match.teamWickets)}</span><span>vs {formatName(match.opponentTeamName)}</span><span>{match.result || "—"}</span></div>;
}
