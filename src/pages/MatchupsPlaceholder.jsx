  ...
  return (
    <div style={container}>
      ...
      <div style={card}>
        ...
        <h2 style={title}>
          {mode === "rivalry" ? "Rivalry Face-Off" : "Head-to-Head Comparison"}
        </h2>
        <p style={sub}>
          {mode === "rivalry"
      ...
      {data && mode === "rivalry" && (
        <div style={resultCard}>
          ...
          <div style={statsGrid}>
            ...
            <div style={statBox}>
              <div style={statVal}>
                {data.totalDismissals >= 5
                  ? "Elite"
                  : data.totalDismissals > 0
                    ? "Strong"
                    : "Clean"}
              </div>
          ...
          {/* DISMISSAL BREAKDOWN */}
          {data.breakdown &&
            Object.values(data.breakdown).some((v) => v > 0) && (
        ...
        (() => {
          ...
          return (
            <div style={comparisonCard}>
              ...
              <div style={comparisonPlayers}>
                <div style={comparisonPlayer}>
                  <div style={avatarCircle}>{(p1 || "A")[0].toUpperCase()}</div>

                  <div style={comparisonPlayerName}>{p1}</div>
                </div>

                <div style={vsPill}>VS</div>

                <div style={comparisonPlayer}>
                  <div style={avatarCircle}>{(p2 || "B")[0].toUpperCase()}</div>

                  <div style={comparisonPlayerName}>{p2}</div>
                </div>
              </div>
              ...
              <div style={statsTable}>
                <StatRow
                  label="Innings"
                  v1={player1.stats?.batting?.innings || 0}
                  v2={player2.stats?.batting?.innings || 0}
                />

                <StatRow
                  label="Runs"
                  v1={player1.stats?.batting?.runs || 0}
                  v2={player2.stats?.batting?.runs || 0}
                  isHighBetter
                />

                <StatRow
                  label="Average"
                  v1={player1.derived?.battingAverage || 0}
                  v2={player2.derived?.battingAverage || 0}
                  isHighBetter
                />

                <StatRow
                  label="SR"
                  v1={player1.derived?.strikeRate || 0}
                  v2={player2.derived?.strikeRate || 0}
                  isHighBetter
                />
              </div>
              ...
              <div style={statsTable}>
                <StatRow
                  label="Wickets"
                  v1={player1.stats?.bowling?.wickets || 0}
                  v2={player2.stats?.bowling?.wickets || 0}
                  isHighBetter
                />

                <StatRow
                  label="Economy"
                  v1={player1.derived?.economy || 0}
                  v2={player2.derived?.economy || 0}
                  isHighBetter={false}
                />

                <StatRow
                  label="MoM"
                  v1={player1.stats?.achievements?.mom || 0}
                  v2={player2.stats?.achievements?.mom || 0}
                  isHighBetter
                />
              </div>
            </div>
...
const StatRow = ({ label, v1, v2, isHighBetter = true }) => {
  ...
  return (
    <div style={rowStyle}>
      ...
      <div style={comparisonLabel}>{label}</div>
