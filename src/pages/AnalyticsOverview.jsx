...
function SpotlightRow({ player, primaryValue, secondaryValue, onOpen }) {
  return (
    <button
      type="button"
  ...
  return (
    <div className={styles.page}>
      ...
      <PerformancePanel
        ...
        secondaryValue={(player) =>
          `${number(player.average == null ? player.totalRuns : player.average)} avg · ${decimal(player.strikeRate)} SR`
        }
      ...
      <PerformancePanel
        ...
        secondaryValue={(player) =>
          `${decimal(player.economyRate)} eco · ${number(player.totalWickets) === 0 || player.average == null ? "—" : decimal(player.average)} avg`
        }
        onOpenPlayer={openPlayer}
      />
    </div>
