  ...
  return (
    <div className={styles.toolbar}>
      <div className={styles.filterSummary} aria-label="Active filters">
        {activeLabels.length === 0 && !warning && <span className={styles.allChip}>All players</span>}
...
export function SortButton({ label, column, activeColumn, direction, onSort, ariaLabel }) {
  ...
  return (
    <button type="button" className={`${styles.sortButton} ${active ? styles.activeSort : ""}`} onClick={() => onSort(column)} aria-label={ariaLabel || `Sort by ${label}`}>
      <span>{label}</span>
      {active && <span className={styles.sortDirection} aria-hidden="true">{direction === "asc" ? "▲" : "▼"}</span>}
    </button>
  );
}
...
export function StatsSkeleton({ rows = 5, compact = false }) {
  return (
    <div className={`${styles.skeletonList} ${compact ? styles.compactSkeleton : ""}`} role="status" aria-label="Loading statistics">
