import { getPlayersForTeam, getMaxWickets, sameName } from "../../../utils/matchModel";
...
export function getMatchInvariantViolations(match) {
  ...
  (match.innings || []).forEach((innings, index) => {
    if (!nonNegative(innings.totalRuns)) issues.push(`innings[${index}].totalRuns is invalid`);
    if (!nonNegative(innings.wickets)) issues.push(`innings[${index}].wickets is invalid`);
    if (!nonNegative(innings.balls)) issues.push(`innings[${index}].balls is invalid`);

    const maxWickets = getMaxWickets(match, innings.battingTeam, Boolean(innings.isSuperOver));
  ...
  if (live) {
    ...
    if (live.striker && live.nonStriker && sameName(live.striker, live.nonStriker)) {
    ...
    if (live.striker && !battingPlayers.some((player) => sameName(player, live.striker))) {
      issues.push("striker is not in the batting squad");
    }
    if (live.nonStriker && !battingPlayers.some((player) => sameName(player, live.nonStriker))) {
      issues.push("non-striker is not in the batting squad");
    }
    if (live.bowler && !bowlingPlayers.some((player) => sameName(player, live.bowler))) {
      issues.push("bowler is not in the bowling squad");
    }
  }

  if (match.result?.winner) {
    const belongsToMatch = [match.teams?.teamA?.name, match.teams?.teamB?.name]
      .some((team) => sameName(team, match.result.winner));
    if (!belongsToMatch) issues.push("result winner is not one of the match teams");
  }
